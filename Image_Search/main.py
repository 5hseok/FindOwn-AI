...
################################################################################################################
root_dir = "C:\\Users\\DGU_ICE\\FindOwn\\extracted_images"
...
with open('features_logo_test.pkl','rb') as f:
    load = pickle.load(f)
for image_path, array in load:
    similar_results_dict.update({image_path:0.0})

#EfficientNet_results
similar_model = models.Image_Search_Model(pre_extracted_features='features_logo_test.pkl')
...
if not os.path.exists('colorHistograms_logo_test.pkl'):
    color_model.save_histograms(root_dir,'colorHistograms_logo_test.pkl')
histograms = color_model.load_histograms('colorHistograms_logo_test.pkl')
...
if not os.path.exists('object_logo_test.pkl'):
    Object_model.create_object_detection_pkl(root_dir,'object_logo_test.pkl')
with open('object_logo_test.pkl','rb') as f:
...
#resnet_results
cnn = models.CNNModel()
if not os.path.exists('cnn_features_test.pkl'):
    cnn.extract_features_from_dir(root_dir, 'cnn_features_test.pkl')
cnn_similarities = cnn.compare_features(target_image_path, 'cnn_features_test.pkl')
